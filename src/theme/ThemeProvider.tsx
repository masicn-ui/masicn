...
import { ThemeContext, type ThemeContextValue, type ThemeMode, type PaletteEntry } from './ThemeContext';
...
export function ThemeProvider({ children, theme: initialTheme = 'system', palettes = [] }: ThemeProviderProps) {
  const systemScheme = useColorScheme();
  const [mode, setMode] = useState<ThemeMode>(initialTheme);
  const [activePalette, setActivePalette] = useState<string>(palettes[0]?.name ?? '');
  ...
  const value = useMemo<ThemeContextValue>(
    () => ({ theme, mode, setMode, toggleTheme, resetToSystem, palettes, activePalette, setPalette }),
    [theme, mode, toggleTheme, resetToSystem, palettes, activePalette, setPalette],
  );
